...
#define ALL_RESULTS_KEY @"GameResults_ALL"  // notice the name is unique don't use ALL
...
- (void)synchronize
{
    // store game results in a dictionary - review dictionary
...
// designated initializer  "iOS common place do not call setters and getters in init"
- (id) init
{
    self = [super init];
    if (self){
        _start = [NSDate date]; // returns right now date
        _end = _start;
    }
    return self;
}
